...
int rtmp_open_for_write(const char *url, uint32_t video_width, uint32_t video_height,uint32_t frame_rate, RTMP** out_rtmp) {
    ...
    *out_rtmp = RTMP_Alloc();
    LIBRTMP_LOGD("rtmp_is_connected out_rtmp %p\n",out_rtmp);
    LIBRTMP_LOGD("rtmp_is_connected &(*out_rtmp) %p\n",&(*out_rtmp));
    RTMP *rtmp = *out_rtmp;
    ...
    LIBRTMP_LOGD("rtmp_open_for_write 1 \n");
    RTMP_Init(rtmp);
    rtmp->Link.receiveTimeoutInMs = RTMP_CONNECTION_TIMEOUT;
    rtmp->Link.sendTimeoutInMs = RTMP_CONNECTION_TIMEOUT;
    RTMPResult ret = RTMP_SetupURL(rtmp, url);

    LIBRTMP_LOGD("rtmp_open_for_write 2 \n");
    if (ret != RTMP_SUCCESS) {
        RTMP_Free(rtmp);
        return ret;
    }
    ...
    LIBRTMP_LOGD("rtmp_open_for_write 3 \n");
    ...
    LIBRTMP_LOGD("rtmp_open_for_write 4 \n");
    ...
    if (RTMP_IsConnected(rtmp)) {
        ...
        *output++ = AMF_ECMA_ARRAY;
        ...
        LIBRTMP_LOGD("rtmp_open_for_write 6 \n");
        memcpy(send_buffer + offset, buffer, body_len);
        LIBRTMP_LOGD("rtmp_open_for_write 7 \n");
...
int send_key_frame(RTMP *rtmp, int nal_len,  uint32_t ts,  uint32_t abs_ts, uint8_t *nal) {
    ...
    if (!output) {
        RTMP_Log(RTMP_LOGDEBUG, "Memory is not allocated...");
    }
    // flv tag header
                                  ...
                                  uint32_t abs_ts)
{
    ...
    if(false){ // 调试
        char buffer[100];  // 假设前10个数据足够存在100字节内
        size_t offset = 0;

        for (size_t i = 0; i < total && i < 10; ++i) {
            offset += sprintf(buffer + offset, "0x%02X ", data[i]);
        }
        LIBRTMP_LOGD("rtmp_sender_write_video_frame buffer:%s",buffer);
    }
    ...
    while (nal != NULL) {
        ...
        if ((nal[0] & 0x1f) == 0x07)  {//it can be 27 67
            if (video_config_ok == true) {
                RTMP_Log(RTMP_LOGDEBUG, "video config is already set");
            ...
            if (nal_n == NULL) {
                RTMP_Log(RTMP_LOGDEBUG, "No Nal after SPS\n");
                return -1;
            }
