...
public:
    ...
    explicit PPOCR_Cls(const std::string &cfg);

    PPOCR_Cls(AAssetManager *mgr, Param &param);

    PPOCR_Cls(AAssetManager *mgr, std::string param, std::string bin);

    PPOCR_Cls(AAssetManager *mgr,
    ...
    void postprocess(ncnn::Mat &outputs, int &class_name);

    void infer(cv::Mat &img, ncnn::Mat &outputs);

    // private:
    ncnn::Net *model{nullptr};
    std::string paramPath = "cls_recognize/model/cls-opt-int8.param";
    std::string binPath = "cls_recognize/model/cls-opt-int8.bin";
