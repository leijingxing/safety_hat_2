...
public:
    ...
    explicit YOLOv8Pose(const std::string &cfg);

    YOLOv8Pose(AAssetManager *mgr, Param &param);

    explicit YOLOv8Pose(AAssetManager *mgr, std::string param, std::string bin);
    ...
    void postprocess(ncnn::Mat &outputs, std::vector<PoseObj> &results);

    void draw_results(cv::Mat &img, std::vector<PoseObj> &detect_results);
    ...
    float kps_thres = {0.45f};
    int numPoint = {3};  // 训练的关键点种类为3
