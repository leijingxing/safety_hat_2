...
static void generate111_proposals(const ncnn::Mat& anchors, int stride, const ncnn::Mat& in_pad, const ncnn::Mat& feat_blob, float prob_threshold, std::vector<Object>& objects)
{
...
static void generate_proposals(const ncnn::Mat &anchors, int stride, const ncnn::Mat &in_pad,
    ...
    const int num_class = feat_blob.c / num_anchors - 5; //19


    const int feat_offset = num_class + 5; //24
    ...
    for (int q = 0; q < num_anchors; q++) {
        ...
        for (int i = 0; i < num_grid_y; i++) {
            for (int j = 0; j < num_grid_x; j++) {
                ...
                float confidence = sigmoid(box_score) * sigmoid(class_score);
//                NCNN_LOGE("box_score = %f",sigmoid(box_score));
//                NCNN_LOGE("class_score = %f",sigmoid(class_score));
//                NCNN_LOGE("confidence = %f",confidence);
...
//        NCNN_LOGE("out0.w = %d",out.w);
//        NCNN_LOGE("out0.h = %d",out.h);
//        NCNN_LOGE("out0.d = %d",out.d);
//        NCNN_LOGE("out0.c = %d",out.c);
//        NCNN_LOGE("out0.dims = %d",out.dims);

//        for(int c = 0; c<out.c; c++){
//            ncnn::Mat out_c = out.channel(c);
//            for(int h = 0; h < out_c.h; h++){
//                for(int w = 0; w < out_c.w; w++){
//                    int index = h * out_c.w + w;
//                    NCNN_LOGE("out = %f",out_c[index]);
//                }
//            }
//        }
