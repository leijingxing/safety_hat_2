...
UVCPreview::UVCPreview(uvc_device_handle_t *devh)
        : mPreviewWindow(NULL),
          ...
          requestShowFps(DEFAULT_PREVIEW_FPS),
...
void UVCPreview::setShowFps(int showfps) {
    requestShowFps = showfps;
}
 ...
 * @return 是否应该显示当前帧
 */
bool shouldDisplayFrame(double timestamp, double targetFps) {
    // 使用静态变量存储状态
    static double lastTimestamp = 0.0;  // 上次显示的帧的时间戳
    static double nextIdealTimestamp = 0.0;  // 下一个理想显示帧的时间戳
    static bool isFirstFrame = true;

    // 目标帧间隔
    const double targetInterval = 1.0 / targetFps;

    // 第一帧总是显示
    if (isFirstFrame) {
        isFirstFrame = false;
        lastTimestamp = timestamp;
        nextIdealTimestamp = timestamp + targetInterval;
        return true;
    }
    ...
    // 核心算法：基于理想时间点的决策
    // 判断当前帧是否是最接近下一个理想显示时间点的帧
    double distanceToIdealTime = timestamp - nextIdealTimestamp;

    // 如果当前帧已经超过了下一个理想显示时间点
    if (distanceToIdealTime >= 0) {
        // 计算新的下一个理想时间点
        // 跳过已经错过的理想时间点，找到未来的下一个
        int missedFrames = (int)(distanceToIdealTime / targetInterval) + 1;
        nextIdealTimestamp += (missedFrames * targetInterval);
    ...
    // 如果当前帧时间比理想时间早，但下一帧会比理想时间晚
    // 计算如果跳过此帧，下一帧的预计时间
    double nextFramePredictedTime = timestamp + (timestamp - lastTimestamp);

    // 如果当前帧比下一帧更接近理想时间点，则显示当前帧
    if (std::abs(distanceToIdealTime) < std::abs(nextFramePredictedTime - nextIdealTimestamp)) {
        // 更新理想时间点
        nextIdealTimestamp += targetInterval;
        lastTimestamp = timestamp;
        return true;
    }
...
//                    double cap_time = (double) frame_mjpeg->capture_time.tv_sec + (double) frame_mjpeg->capture_time.tv_usec/ (double)1000000;
                    double cap_time_f = (double) frame_mjpeg->capture_time_finished.tv_sec + (double) frame_mjpeg->capture_time_finished.tv_nsec/ (double)1000000000;

                    // requestShowFps 根据设置的帧率，判断是否需要解码渲染
                    bool showFlag = shouldDisplayFrame(cap_time_f, requestShowFps);
                    if(showFlag){
                        frame = get_frame(
                                frame_mjpeg->width * frame_mjpeg->height * PREVIEW_PIXEL_BYTES);

//                        LOGI("uvc_mjpeg2yuyv capture_time: %f  capture_time_finished:%f ", cap_time, cap_time_f);
//                    c_start = clock();
                        result = uvc_mjpeg2rgbx_tj(frame_mjpeg, frame);   // MJPEG => yuyv
//                    c_end = clock();
//                    LOGI("uvc_mjpeg2yuyv time: %f  result:%d data_bytes:%zu capacity_bytes:%zu", (double) (c_end - c_start) / CLOCKS_PER_SEC, result,
//                         frame_mjpeg->data_bytes, frame_mjpeg->capacity_bytes);
//                    result = 0;
                        if (LIKELY(!result)) {
