...
public class UVCCamera {
    ...
    static {
        ...
        System.loadLibrary("usb1.9");
        System.loadLibrary("uvc1.9");
...
//            return sizeList.get(0);
    ...
    public void setShowFps(int showFps) {
        if ((showFps < 0)) {
            throw new IllegalArgumentException("invalid show fps");
        }

        if (mNativePtr != 0) {
            //set preview size
            nativeSetShowFps(mNativePtr, showFps);
        }
    }
...
//        setFrameCallback(null, 0);
    ...
    public native void nativeSetShowFps(final long id_camera, int showFps);
}
